...
class NativeBannerScrollableListViewController: UICollectionViewController {
    ...
    private var banners = [NativeBannerEntity]()

    private var bannerViews = [String: NativeBannerView]()
...
extension NativeBannerScrollableListViewController {
    ...
    private func banner(for indexPath: IndexPath) -> NativeBannerEntity {
        return banners[(indexPath.section + 1) * indexPath.row]
    }

    private func bannerView(for banner: NativeBannerEntity) -> NativeBannerView? {
...
extension NativeBannerScrollableListViewController: BannerViewDelegate {
...
extension NativeBannerScrollableListViewController: UICollectionViewDelegateFlowLayout {
...
struct NativeBannerEntity: Identifiable {
    var id: String
    ...
    private func fetchBanners(block: @escaping ([NativeBannerEntity], Error?) -> Void) {
        ...
        DispatchQueue.global(qos: .utility).async {
            ...
            for i in 0..<num {
                ...
                result.append(NativeBannerEntity(
                    id: id,
                    query: query
                ))
            }
