...
class SingleProgrammaticallyWebBannerViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        let pxBannerHeight = Int(bannerView.frame.size.height * mainScreenScale)
        bannerView.query = BannerCreativeQuery(
            placementId: "YOUR_PLACEMENT_ID",
            closeButtonType: .COUNTDOWN(timeout: 5.0),
            sizes: [
                SizeEntity(width: 260, height: 106),
                SizeEntity(width: pxBannerWidth, height: pxBannerHeight)
            ],
            floorPrice: 1.99,
            currency: "USD",
            customParams: [
